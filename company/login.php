...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($email_err) && empty($password_err)) {
    $stmt = $conn->prepare("SELECT company_id, company_name, password, status FROM transport_companies WHERE email = ?");
    $stmt->execute([$email]);
    $company = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($company) {
      if ($company["status"] !== "active") {
        $login_err = "Account is not active. Please contact support.";
...
<head>
  ...
  <title>Transport Company Login</title>
...
<body>
  <div class="login-card">
    <h3><i class=""></i>Transport Company Login</h3>
